...
function grabCake() {
    ...
    // 轮询
    var timer = setInterval(autoOrder, 2000);
}
...
function autoOrder() {
    if(config.isOrderring) 
    {
        console.log('仍在交易中，轮询直接返回。。。。');
        return;
    }
    chrome.storage.local.get('distance', function (store) {
        console.log('config distance:' + store.distance);
    ...
    if (btccnt >= 0.1 && parseInt(chajia) > parseInt(config.distance)) {
        ...
        if(isInArray(arrBuy[arrBuy.length - 1],config.boughts))
        {
            console.log('已经买过这单，不再购买');
            return;
        }
        config.isOrderring=true;
        console.log('差价出现了:' + chajia + ",时间：" + new Date().toString());
        config.cnt++;
        ...
        if ($btn) {
            ...
            setTimeout(() => {
                ...
                console.log('已自动下单,时间：' + new Date().toString());
                config.boughts.push(arrBuy[arrBuy.length - 1]);
                config.isOrderring=false;
                console.log('重置isorderring为false');
            }, 500);
...
{
    return JSON.stringify(array).indexOf(JSON.stringify(a))!=-1;
}
